...
# ================= SERVICE PAGES =================
@app.route("/general")
def general(): return render_template("general.html")

@app.route("/physiotherapy")
def physiotherapy(): return render_template("physiotherapy.html")

@app.route("/nursing")
def nursing(): return render_template("nursing.html")

@app.route("/lab")
def lab(): return render_template("lab.html")

@app.route("/fullbody")
def fullbody(): return render_template("fullbody.html")

@app.route("/elder")
def elder(): return render_template("elder.html")

@app.route("/child")
def child(): return render_template("child.html")

@app.route("/diabetic")
def diabetic(): return render_template("diabetic.html")

@app.route("/pregnent")
def pregnent(): return render_template("pregnent.html")
...
def book_service():
    ...
    cursor.execute(
        """INSERT INTO bookings 
           (name, phone, service, date, doctor, username, status)
...
def check_status():
    ...
    cursor = db.cursor(dictionary=True)
    cursor.execute(
        "SELECT id FROM bookings WHERE username=%s AND status='Accepted' LIMIT 1",
        (session["user"],)
    )
    accepted = cursor.fetchone()
    cursor.close()

    return jsonify({"accepted": True if accepted else False})
